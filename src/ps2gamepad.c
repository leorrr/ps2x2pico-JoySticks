...
//disparo boton B
       if (report[5]==79 && joy1B_state==1){kb_send_key(0x2b, 1, 0);}          
       else if (report[5]==15 && joy1B_state==1){kb_send_key(0x2b, 0, 0);joy1B_state=0;}
       if (report[5]==79 && joy1B_state==0){kb_send_key(0x2b, 1, 0);joy1B_state=1;}
...
//ESCAPE boton A
       if (report[5]==47 && joy1A_state==1){kb_send_key(0x29, 1, 0);}          
       else if (report[5]==15 && joy1A_state==1){kb_send_key(0x29, 0, 0);joy1A_state=0;}
       if (report[5]==47 && joy1A_state==0){kb_send_key(0x29, 1, 0);joy1A_state=1;}

//arriba boton X
       if (report[5]==31 && joy1X_state==1){kb_send_key(0x52, 1, 0);} 
       else if (report[5]==15 && joy1X_state==1){kb_send_key(0x52, 0, 0);joy1X_state=0;}
       if (report[5]==31 && joy1X_state==0){kb_send_key(0x52, 1, 0);joy1X_state=1;}  

//ESCAPE boton Y
       if (report[5]==143 && joy1Y_state==1){kb_send_key(0x29, 1, 0);}          
       else if (report[5]==15 && joy1Y_state==1){kb_send_key(0x29, 0, 0);joy1Y_state=0;}
       if (report[5]==143 && joy1Y_state==0){kb_send_key(0x29, 1, 0);joy1Y_state=1;}

//F1 boton Select
       if (report[6]==16 && joy1Select_state==1){kb_send_key(0x3A, 1, 0);} 
       else if (report[6]==0 && joy1Select_state==1){kb_send_key(0x3A, 0, 0);joy1Select_state=0;}
       if (report[6]==16 && joy1Select_state==0){kb_send_key(0x3A, 1, 0);joy1Select_state=1;} 

//Enter boton Start
       if (report[6]==32 && joy1Start_state==1){kb_send_key(0x28, 1, 0);} 
       else if (report[6]==0 && joy1Start_state==1){kb_send_key(0x28, 0, 0);joy1Start_state=0;}
       if (report[6]==32 && joy1Start_state==0){kb_send_key(0x28, 1, 0);joy1Start_state=1;}        

//F2 boton Trigger L
       if (report[6]==1 && joy1Ltrigger_state==1){kb_send_key(0x3B, 1, 0);} 
       else if (report[6]==0 && joy1Ltrigger_state==1){kb_send_key(0x3B, 0, 0);joy1Ltrigger_state=0;}
       if (report[6]==1 && joy1Ltrigger_state==0){kb_send_key(0x3B, 1, 0);joy1Ltrigger_state=1;} 

 //F5 boton Trigger R
       if (report[6]==2 && joy1Rtrigger_state==1){kb_send_key(0x3E, 1, 0);} 
       else if (report[6]==0 && joy1Rtrigger_state==1){kb_send_key(0x3E, 0, 0);joy1Rtrigger_state=0;}
       if (report[6]==2 && joy1Rtrigger_state==0){kb_send_key(0x3E, 1, 0);joy1Rtrigger_state=1;}       
